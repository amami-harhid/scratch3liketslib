...
export class Costumes {
   ...
   async addImage(name:string, image:any) :Promise<void>{
        await this._setSkin(name, image);
        await Utils.wait(Env.pace);
    }
    async loadImage(name:string, image:string) :Promise<void> {
        const _img = await ImageLoader.loadImage(image, name);
        this.addImage(name,_img);
    }
    async _setSkin(name:string,_img:any) :Promise<void> {
    ...
    async _setSvgSkin(_svgText:string) :Promise<number>{
    ...
    async _setBitmapSkin(_bitmap: object) :Promise<number>{
        if(this.render && this.render.renderer){
    ...
    update( drawableID: string, effect = {} ): void {
        if(this.render && this.render.renderer){
