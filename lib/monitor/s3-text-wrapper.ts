...
export class S3TextWrapper {
     ...
     */
    constructor (measurementProvider: S3CanvasMeasurementProvider) {
     ...
     */
    wrapText (maxWidth:number, text:string) {
        ...
        while ((nextBreak = breaker.nextBreak())) {
            ...
            let proposedLine:string = (currentLine || '').concat(word);
            let proposedLineWidth:number = this._measurementProvider.measureText(proposedLine);
